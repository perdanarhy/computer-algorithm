...
// Find the slide deck file whose name starts with this week's prefix
// (e.g. "week01" -> "week01-introduction.md"), so materials pages link
// back to the correct deck regardless of its topic-slug suffix.
function slideFileFor(week) {
  if (!fs.existsSync(slidesDir)) return null;
  const match = fs.readdirSync(slidesDir).find(
    (f) => f.startsWith(week + '-') && f.endsWith('.md'),
  );
  return match ? match.replace(/\.md$/, '.html') : null;
}
...
function renderPage({ title, week, body }) {
  const slideFile = slideFileFor(week);
  const backLink = slideFile
    ? `<a href="../../${slideFile}">&larr; Back to ${weekLabel(week)} slides</a>`
